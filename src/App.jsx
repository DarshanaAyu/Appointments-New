...
// Shared by QuickAddPanel (new bookings) and ApptDetailPanel (rescheduling):
// available start times for a given date+duration, optionally excluding one appointment (itself, when editing).
function timeSlotsFor(allAppointments, shiftSettings, dateKey, duration, excludeId) {
  const shift = shiftFor(shiftSettings, dateKey);
  if (shift.closed) return [];
  const others = allAppointments.filter((a) => a.dateKey === dateKey && a.id !== excludeId);
  const slots = [];
...
function durationMaxFor(allAppointments, shiftSettings, dateKey, startMin, excludeId) {
  const shift = shiftFor(shiftSettings, dateKey);
  const others = allAppointments.filter((a) => a.dateKey === dateKey && a.id !== excludeId);
...
function ScheduleView({ appointments, setAppointments, clients, setClients, shiftSettings, prefillClient, onConsumePrefill }) {
  ...
  async function handleSave({ dateKey: apptDateKey, startMin: apptStartMin, duration, name, phone, notes, wantsWhatsapp, wantsSms, visitType, clientId }) {
    playChime("booked");
    const finalClientId = clientId || uid();
    const appt = {
      id: uid(), serviceName: SERVICE_NAME, duration, dateKey: apptDateKey, startMin: apptStartMin,
      clientId: finalClientId,
      clientName: name.trim(), clientPhone: phone.trim(), notes: notes.trim(),
      visitType: visitType || "walkin",
      reminders: { whatsapp: wantsWhatsapp, sms: wantsSms, whatsappSent: false, smsSent: false },
  ...
  async function handleMarkSent(id, method) {
    const key = method === "whatsapp" ? "whatsappSent" : "smsSent";
    await setAppointments(appointments.map((a) => a.id === id ? { ...a, reminders: { ...a.reminders, [key]: true } } : a));
  }

  if (quickAddStart != null) {
    return <QuickAddPanel dateKey={dateKey} startMin={quickAddStart} allAppointments={appointments} shiftSettings={shiftSettings} clients={clients} prefill={pendingPrefill}
      onCancel={() => { setQuickAddStart(null); setPendingPrefill(null); }} onSave={handleSave} />;
  }

  if (viewingId) {
    const appt = appointments.find((a) => a.id === viewingId);
    if (appt) return <ApptDetailPanel appt={appt} allAppointments={appointments} shiftSettings={shiftSettings} onBack={() => setViewingId(null)} onDelete={() => handleDelete(appt.id)} onCheckout={(price) => handleCheckout(appt.id, price)} onUndoCheckout={() => handleUndoCheckout(appt.id)} onMarkSent={(method) => handleMarkSent(appt.id, method)} onUpdate={(patch) => handleUpdate(appt.id, patch)} />;
  }
...
function QuickAddPanel({ dateKey, startMin, allAppointments, shiftSettings, clients, prefill, onCancel, onSave }) {
  const [pendingDate, setPendingDate] = useState(dateKey);
  const [pendingStart, setPendingStart] = useState(startMin);
  const [editing, setEditing] = useState(null); // 'date' | 'time' | null
  const [dateNeedsTime, setDateNeedsTime] = useState(false);

  const sliderMax = durationMaxFor(allAppointments, shiftSettings, pendingDate, pendingStart);
  const [duration, setDuration] = useState(Math.min(30, sliderMax) || sliderMax);

  // If moving the date/time shrinks the available room, pull the duration back down to fit.
  useEffect(() => {
    const max = durationMaxFor(allAppointments, shiftSettings, pendingDate, pendingStart);
    if (duration > max) setDuration(Math.min(30, max) || max);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pendingDate, pendingStart]);
  ...
  function changeDate(newDate) {
    ...
    const slots = timeSlotsFor(allAppointments, shiftSettings, newDate, duration);
    if (slots.includes(pendingStart)) {
      setEditing(null);
    ...
    <div className="p-5">
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          ...
          <div className="text-xs mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-1" style={{ color: C.inkMuted }}>
            <span>{SERVICE_NAME} ·</span>
            <button onClick={() => { setDateNeedsTime(false); setEditing("date"); }} className="underline decoration-dotted underline-offset-2" style={{ color: C.forest }}>{fullDateLabel(pendingDate)}</button>
            <span>·</span>
            <button onClick={() => setEditing("time")} className="underline decoration-dotted underline-offset-2" style={{ color: C.forest }}>{minutesToLabel(pendingStart)}</button>
          </div>
          {prefill && <div className="text-xs mt-1 font-medium" style={{ color: C.forest }}>Booking for {prefill.name} — first open slot picked automatically, change it above if needed.</div>}
        </div>
        <button onClick={onCancel}><X size={18} color={C.inkMuted} /></button>
      </div>

      {editing === "date" && (
        <div className="rounded-2xl p-3 mt-3" style={{ backgroundColor: C.sageLight }}>
          ...
          <input
            ...
            className="w-full rounded-lg p-2.5 text-sm outline-none border mb-2" style={{ borderColor: C.line, backgroundColor: "white", color: C.ink }}
          />
          {dateNeedsTime && (
            <>
              <div className="text-xs mb-2" style={{ color: C.danger }}>
                {shiftFor(shiftSettings, pendingDate).closed
                  ...
                  : `${minutesToLabel(pendingStart)} isn't free on ${shortDateLabel(pendingDate)} — pick a new time:`}
              </div>
              <div className="max-h-40 overflow-y-auto space-y-1 pr-0.5">
                {timeSlotsFor(allAppointments, shiftSettings, pendingDate, duration).map((m) => (
                  <button key={m} onClick={() => pickTime(m)}
                ...
                {timeSlotsFor(allAppointments, shiftSettings, pendingDate, duration).length === 0 && (
                  <div className="text-xs" style={{ color: C.inkMuted }}>No free slots long enough on this date.</div>
                )}
              </div>
              <button onClick={() => { setPendingDate(dateKey); setDateNeedsTime(false); setEditing(null); }} className="mt-2 text-xs" style={{ color: C.inkMuted }}>Cancel — keep original date</button>
            </>
          )}
          {!dateNeedsTime && (
            <button onClick={() => setEditing(null)} className="mt-2 text-xs" style={{ color: C.inkMuted }}>Done</button>
          )}
        </div>
      )}
          ...
          <div className="max-h-40 overflow-y-auto space-y-1 pr-0.5">
            {timeSlotsFor(allAppointments, shiftSettings, pendingDate, duration).map((m) => (
              <button key={m} onClick={() => pickTime(m)}
                ...
                style={{ borderColor: m === pendingStart ? C.forest : C.line, backgroundColor: "white", color: C.ink, fontWeight: m === pendingStart ? 600 : 400 }}>
                {minutesToLabel(m)}{m === pendingStart ? " · current" : ""}
              </button>
            ))}
            {timeSlotsFor(allAppointments, shiftSettings, pendingDate, duration).length === 0 && (
              <div className="text-xs" style={{ color: C.inkMuted }}>No free slots this long on this date.</div>
            )}
          </div>
      ...
      {sliderMax < SLIDER_MIN ? (
        <div className="mt-4 text-xs" style={{ color: C.danger }}>Not enough time on this date — try an earlier slot or a different date.</div>
      ) : (
        <div className="mt-5">
      ...
      <div className="mt-6">
        <PrimaryButton full disabled={!canSave} onClick={() => onSave({ dateKey: pendingDate, startMin: pendingStart, duration, name, phone, notes, wantsWhatsapp: wantsWhatsapp && !!phone.trim(), wantsSms: wantsSms && !!phone.trim(), visitType, clientId: selectedClientId })}>Save appointment</PrimaryButton>
      </div>
    </div>
  );
}
...
function ApptDetailPanel({ appt, allAppointments, shiftSettings, onBack, onDelete, onCheckout, onUndoCheckout, onMarkSent, onUpdate }) {
  ...
  function timeSlots(dk, duration) {
    return timeSlotsFor(allAppointments, shiftSettings, dk, duration, appt.id);
  }
  function durationMax(dk, startMin) {
    return durationMaxFor(allAppointments, shiftSettings, dk, startMin, appt.id);
  }
  ...
  return (
    <div className="p-5">
      ...
      <div className="flex gap-1.5 mt-3 flex-wrap">
        ...
        {appt.reminders.whatsapp && <Pill tone={appt.reminders.whatsappSent ? "turmeric" : undefined}>WhatsApp{appt.reminders.whatsappSent ? " sent" : ""}</Pill>}
        {appt.reminders.sms && <Pill tone={appt.reminders.smsSent ? "turmeric" : undefined}>Text{appt.reminders.smsSent ? " sent" : ""}</Pill>}
        ...
        <div className="flex gap-2 mt-3">
          {appt.reminders.whatsapp && (
            <button onClick={() => sendVia("whatsapp")} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium"
              ...
              {appt.reminders.whatsappSent ? <Check size={13} /> : <MessageCircle size={13} />} {appt.reminders.whatsappSent ? "Resend WhatsApp" : "Send WhatsApp"}
            </button>
          )}
          {appt.reminders.sms && (
            <button onClick={() => sendVia("sms")} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium"
              ...
              {appt.reminders.smsSent ? <Check size={13} /> : <Bell size={13} />} {appt.reminders.smsSent ? "Resend Text" : "Send Text"}
            </button>
          )}
        </div>
      )}
      ...
      <div className="mt-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          <input type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)}
            className="flex-1 rounded-xl p-2.5 text-sm outline-none border" style={{ borderColor: C.line, backgroundColor: "white", color: C.ink }} />
              ...
              {!s.closed && (
                <div className="flex items-center gap-2 mt-2">
                  <input
                    ...
                    className="flex-1 rounded-lg p-2 text-sm outline-none border" style={{ borderColor: C.line, backgroundColor: "white", color: C.ink }}
                  />
...
function ReminderQueue({ appointments, setAppointments }) {
  ...
  const upcoming = appointments.filter((a) => {
    ...
    const needsWhatsapp = a.reminders.whatsapp && !a.reminders.whatsappSent;
    const needsSms = a.reminders.sms && !a.reminders.smsSent;
    if (!needsWhatsapp && !needsSms) return false;
  ...
  async function markSent(id, method) {
    const key = method === "whatsapp" ? "whatsappSent" : "smsSent";
    await setAppointments(appointments.map((a) => (a.id === id ? { ...a, reminders: { ...a.reminders, [key]: true } } : a)));
  }
  ...
  return (
    <div className="p-5">
      ...
      <p className="text-xs mt-1.5 leading-relaxed" style={{ color: C.inkMuted }}>
        ...
        A reminder stays here, marked sent, until every channel you picked for it has actually been sent, so you
        can come back and send the rest. For fully automatic or one-tap bulk sending, a messaging provider like
        Twilio or the WhatsApp Business API would need to be connected on a backend.
      </p>
                  ...
                  <div className="flex gap-2 mt-2">
                    {a.reminders.whatsapp && (
                      <button onClick={() => sendVia("whatsapp", a)} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium"
                        style={{ backgroundColor: C.sageLight, color: C.forest }}>
                        {a.reminders.whatsappSent ? <Check size={13} /> : <MessageCircle size={13} />} {a.reminders.whatsappSent ? "Resend WhatsApp" : "Send WhatsApp"}
                      </button>
                    )}
                    {a.reminders.sms && (
                      <button onClick={() => sendVia("sms", a)} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium"
                        style={{ backgroundColor: "#F1E3C4", color: C.turmericDark }}>
                        {a.reminders.smsSent ? <Check size={13} /> : <Bell size={13} />} {a.reminders.smsSent ? "Resend Text" : "Send Text"}
                      </button>
                    )}
                  </div>
                </div>
