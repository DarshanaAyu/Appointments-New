...
import {
  ...
  Search, Bell, MessageCircle, Users, LayoutDashboard, ArrowLeft, X, Settings
} from "lucide-react";
import {
...
function Field({ icon, placeholder, value, onChange, type = "text", inputMode, autoComplete = "off" }) {
  return (
        ...
        {!ready ? (
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex border-b overflow-x-auto flex-shrink-0" style={{ borderColor: C.line }}>
              ...
              <StudioTab icon={<Settings size={14} />} label="Settings" active={tab === "settings"} onClick={() => setTab("settings")} />
            </div>
            <div className="flex-1 overflow-y-auto min-h-0">
              ...
              {tab === "settings" && <ShiftSettings shiftSettings={shiftSettings} setShiftSettings={setShiftSettings} />}
            </div>
          </div>
        )}
      </div>
...
/* ==================================================================== */
function ScheduleView({ appointments, setAppointments, clients, setClients, shiftSettings }) {
  ...
  if (quickAddStart != null) {
    return <QuickAddPanel dateKey={dateKey} startMin={quickAddStart} dayAppts={dayAppts} clients={clients} closeMin={dayShift.close} onCancel={() => setQuickAddStart(null)} onSave={handleSave} />;
  }
...
function QuickAddPanel({ dateKey, startMin, dayAppts, clients, closeMin, onCancel, onSave }) {
    ...
    <div className="p-5">
      ...
      <div className="mt-5 space-y-2">
        <PatientField
          ...
          onSelectClient={(c) => { setName(c.name); setPhone(c.phone); setSelectedClientId(c.id); }}
          onAddNew={(typedName) => { setName(typedName); setSelectedClientId(null); }}
        />
        <Field icon={<Phone size={15} />} placeholder="Phone number (optional)" value={phone} type="tel" inputMode="tel"
        ...
        <div className="px-3 pb-3 space-y-3">
          {editing ? (
            <div className="space-y-2">
              ...
              <Field icon={<Phone size={15} />} placeholder="Phone number" value={phone} onChange={setPhone} type="tel" inputMode="tel" />
              <div className="flex gap-2">
...
/* SETTINGS                                                              */
/* ==================================================================== */
function ShiftSettings({ shiftSettings, setShiftSettings }) {
  ...
  return (
    <div className="p-5">
      <div className="serif text-xl" style={{ color: C.forest }}>Shift hours</div>
